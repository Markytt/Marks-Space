...
def automatic():
  ...
  while goal == False:
    ...
    if randomNumber in [0,1,2]:
      generate(playerY, playerY+1, playerX+1, playerX+2, SAND)
    elif randomNumber in [3,4] and playerY < MAPHEIGHT - 2:
      generate(playerY+1, playerY+2, playerX, playerX+1, SAND)
    else:
      i = 0
      while i < 5:
        generate(playerY-1, playerY, playerX, playerX+1, SAND)
        moveUp()
        i = i+1
    if (moveRight()):
      moveRight()
...
def generateWorld():
  ...
  #     moveDown()
  #   else:
  #       generate(playerX, playerX+1, playerY+1, playerY+2, SAND)
